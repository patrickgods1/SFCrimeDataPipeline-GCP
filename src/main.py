...
def csvToParquet(filename, bucket_name):
    """
    Use the Python dask library to read the CSV formatted data and save as a parquet file in the
    parquet bucket in GCS.
    """
    # df = dd.read_csv(f'gcs://{bucket_name}/raw/{csv_file}.csv', sep='|', #blocksize="10MB", 
    df = pd.read_csv(f'gcs://{bucket_name}/raw/{filename}.csv', sep='|', 
                parse_dates=['Incident Datetime', 'Incident Date', 'Incident Time', 'Report Datetime'], 
    ...
    schema = pa.schema([pa.field('Incident Datetime', pa.timestamp('s', tz='America/Los_Angeles')),
                        ...
                        pa.field('Central Market/Tenderloin Boundary Polygon - Updated', pa.int64()),
    ...
    df.to_parquet(f'gcs://{bucket_name}/{filename}.parquet', engine='pyarrow', schema=schema)
